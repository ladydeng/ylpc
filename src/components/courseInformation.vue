<template>
    <div>
        ...
        <div>
            <div class="btn">
                <a-button @click="addDictionary" type="primary">添加授课基本信息</a-button>
                <!-- <div class="idSearch">
            ...
            <a-modal v-model="performanceVisible" :title="isAdd ? '添加绩效工资核算内容字典':'修改绩效工资核算内容字典'" :rules="rules" @ok="handleOk">
                    <a-form-model :rules="rules"  ref="formData" :model="formData" :label-col="{ span: 7 }" :wrapper-col="{ span: 14 }">
                        ...
                        <a-form-model-item prop="content" required label="授课内容">
                            <a-input v-model="formData.content" />
                        </a-form-model-item>
                        ...
                        <a-form-model-item prop="giveMethod" required label="课程类别">
                            <a-select v-model="formData.giveMethod" style="width: 275px" @change="handleChange">
                        ...
                        <a-form-model-item prop="giveObject" required label="授课对象">
                            <a-input v-model="formData.giveObject" />
                        </a-form-model-item>
                        <a-form-model-item prop="giveDate" required label="授课时间">
                            <a-input v-model="formData.giveDate" />
                        </a-form-model-item>
                        <a-form-model-item prop="giveTeacher" required label="授课老师">
        ...
        <!-- 表格 -->
        <a-table :rowKey="record=>record.id" :columns="columns" :data-source="courseInforTable">
...
const columns = [
    ...
    { title: "授课内容", dataIndex: "content", key: "content" },
    { title: "授课对象", dataIndex: "giveObject", key: "giveObject" },
    { title: "授课老师", dataIndex: "giveTeacher", key: "giveTeacher" },
    { title: "授课方式", dataIndex: "giveMethod", key: "giveMethod" },
    { title: "授课时间", dataIndex: "giveDate", key: "giveDate" },
   ...
   */
    ...
    data() {
        return {
            ...
            rules:{
                content:[{ required: true, message: "更改不能为空", trigger: "blur" }],
                giveDate:[{validator: validatorDate,trigger:"blur"}],      // 更改时间
                // effectiveDate:[{validator: validatorDate,trigger:"blur"}],      // 更改时间
                giveObject:[{ required: true, message: "更改内容不能为空", trigger: "blur" }],
                giveTeacher:[{ required: true, message: "更改倍数不能为空", trigger: "blur" }],
                giveMethod:[{ required: true, message: "更改类型不能为空", trigger: "blur" }],
    ...
    methods:{
        ...
        // 获取绩效工资核算内容历史数据数据列表
        getcourseInfomation() {
            axios.get('http://192.168.90.21:7999/teaching-management/teaching/findAll').then(res=>{
                console.log("data:",res);
                this.courseInforTable = res.data
                // console.log("this.bank:" , this.courseInforTable)
            })
        },
        // 点击按钮添加绩效工资内容词典
        addDictionary() {
            ...
            this.formData.giveDate = '',
            this.formData.giveMethod = '',
            this.formData.giveObject = '',
            this.formData.giveTeacher = ''
        ...
        handleOk() {
            ...
            this.$refs['formData'].validate(valid=>{
                if(!valid) {
                    ...
                    if(this.isAdd) {
                        // 添加绩效工资核算
                        axios.post('http://192.168.90.21:7999/teaching-management/teaching/insert',qs.stringify(this.formData))
                        .then(res=>{
                            if(res.status == 200) {
                                // console.log(res);
                        ...
                        // 获取修改信息
                        axios.post('http://192.168.90.21:7999/teaching-management/teaching/update',qs.stringify(this.formData))
                        .then(res=>{
                            if(res.status == 200) {
                                console.log('数据', res);
                                this.performanceVisible = false
        ...
        // 根据员工id获取绩效工资核算历史信息
        idSearch() {
            axios.get('http://192.168.90.21:7999/teaching-management/teaching/finById?id='+this.id).then(res=>{
        ...
        editor(e) {
            ...
            this.formData.giveDate = e.giveDate,
            this.formData.giveMethod = e.giveMethod,
            this.formData.giveObject = e.giveObject,
            this.formData.giveTeacher = e.giveTeacher
        ...
        del(e) {
            console.log(e);
            if(confirm('确认删除吗？')) {
                axios.post('http://192.168.90.21:7999/teaching-management/teaching/delete','id='+e.id).then(res=>{
                    // console.log(res);
                    if(res.status ==200) {
                        this.getcourseInfomation()
                    }
